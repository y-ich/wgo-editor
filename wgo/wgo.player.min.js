...
a){for(var d=a,p,e=1;e<b.length;e++)if(b[e].constructor==Array)for(var g=0;g<b[e].length;g++)p=new u(b[e][g][0]),d.appendChild(p),k(b[e][g],p);else p=new u(b[e]),d.insertAfter(p),d=p},b=function(b){return"string"==typeof b?b.replace(/\\/g,"\\\\").replace(/]/g,"\\]"):b},a=function(b,a){return String.fromCharCode(97+b)+String.fromCharCode(97+a)},r=function(b,a,d){if(a.length){d.sgf+=b;for(var p in a)d.sgf+="["+a[p]+"]"}},g=function(d,e){if(d.move){var p="";d.move.pass||(p=a(d.move.x,d.move.y));e.sgf=
d.move.c==c.B?e.sgf+("B["+p+"]"):e.sgf+("W["+p+"]")}if(d.setup){var p=[],A=[],k=[],m;for(m in d.setup)d.setup[m].c==c.B?p.push(a(d.setup[m].x,d.setup[m].y)):d.setup[m].c==c.W?A.push(a(d.setup[m].x,d.setup[m].y)):k.push(a(d.setup[m].x,d.setup[m].y));r("AB",p,e);r("AW",A,e);r("AE",k,e)}if(d.markup){p={};for(m in d.markup)p[d.markup[m].type]=p[d.markup[m].type]||[],"LB"==d.markup[m].type?p.LB.push(a(d.markup[m].x,d.markup[m].y)+":"+b(d.markup[m].text)):p[d.markup[m].type].push(a(d.markup[m].x,d.markup[m].y));
for(var l in p)r(l,p[l],e)}m=d.getProperties();for(l in m)"object"!=typeof m[l]&&(e.sgf="turn"==l?e.sgf+("PL["+(m[l]==c.B?"B":"W")+"]"):"comment"==l?e.sgf+("C["+b(m[l])+"]"):e.sgf+(l+"["+b(m[l])+"]"));if(1==d.children.length)e.sgf+="\n;",g(d.children[0],e);else if(1<d.children.length)for(l in d.children)m=d.children[l],p=e,p.sgf+="(\n;",g(m,p),p.sgf+="\n)"},l=function(){this.size=19;this.info={};this.root=new u;this.propertyCount=this.nodeCount=0};l.prototype={constructor:l,clone:function(){var d=
...
break;case "WR":this.kifu.info.white.rank=b.target.innerText;break;default:this.kifu.info[b.target.dataset.sgfTag]=b.target.innerText}this.kifu._edited=!0}},k=c.extendClass(c.Player,function(b,a){this.config=a;for(var r in k["default"])void 0===this.config[r]&&void 0!==k["default"][r]&&(this.config[r]=k["default"][r]);for(r in c.Player["default"])void 0===this.config[r]&&void 0!==c.Player["default"][r]&&(this.config[r]=c.Player["default"][r]);this.element=b;this.element.innerHTML="";this.classes=
...
this.addEventListener("input",q.bind(this))});k.prototype.appendTo=function(b){b.appendChild(this.element);this.updateDimensions()};k.prototype.updateDimensions=function(){for(var b=window.getComputedStyle(this.element),a=[];this.element.firstChild;)a.push(this.element.firstChild),this.element.removeChild(this.element.firstChild);for(var c=parseInt(b.width),g=parseInt(b.height),k=parseInt(b.maxHeight)||0,b=0;b<a.length;b++)this.element.appendChild(a[b]);if(c!=this.width||g!=this.height||k!=this.maxHeight){this.width=
c;this.height=g;this.maxHeight=k;a:if(a=this.config.layout,a.constructor==Array){c=this.height||this.maxHeight;for(g=0;g<a.length;g++)if(!a[g].conditions||!(a[g].conditions.minWidth&&!(a[g].conditions.minWidth<=this.width)||a[g].conditions.minHeight&&c&&!(a[g].conditions.minHeight<=c)||a[g].conditions.maxWidth&&!(a[g].conditions.maxWidth>=this.width)||a[g].conditions.maxHeight&&c&&!(a[g].conditions.maxHeight>=c)||a[g].conditions.custom&&!a[g].conditions.custom.call(this))){a=a[g];break a}a=void 0}if((this.currentLayout=
...
this.dom.board.clientWidth;(e=this.height||this.maxHeight)&&(e-=this.regions.top.element.offsetHeight+this.regions.bottom.element.offsetHeight);e&&e<a?e!=this.board.height&&this.board.setHeight(e):a!=this.board.width&&this.board.setWidth(a);a=e-a;0<a?(this.dom.board.style.height=e+"px",this.dom.board.style.paddingTop=a/2+"px"):(this.dom.board.style.height="auto",this.dom.board.style.paddingTop="0");this.regions.left.element.style.height=this.dom.center.offsetHeight+"px";this.regions.right.element.style.height=
this.dom.center.offsetHeight+"px";for(b in this.components)this.components[b].updateDimensions&&this.components[b].updateDimensions()}};k.prototype.showMessage=function(b,a,r){this.info_overlay=document.createElement("div");this.info_overlay.style.width=this.element.offsetWidth+"px";this.info_overlay.style.height=this.element.offsetHeight+"px";this.info_overlay.className="wgo-info-overlay";this.element.appendChild(this.info_overlay);var g=document.createElement("div");g.className="wgo-info-message";
g.innerHTML=b;b=document.createElement("div");b.className="wgo-info-close";r||(b.innerHTML=c.t("BP:closemsg"));g.appendChild(b);this.info_overlay.appendChild(g);a?this.info_overlay.addEventListener("click",function(b){a(b)}):r||this.info_overlay.addEventListener("click",function(a){this.hideMessage()}.bind(this));this.setFrozen(!0)};k.prototype.hideMessage=function(){this.element.removeChild(this.info_overlay);this.setFrozen(!1)};k.prototype.error=function(b){if(!c.ERROR_REPORT)throw b;switch(b.name){case "InvalidMoveError":this.showMessage("<h1>"+
b.name+"</h1><p>"+b.message+'</p><p>If this message isn\'t correct, please report it by clicking <a href="#">here</a>, otherwise contact maintainer of this site.</p>');break;case "FileError":this.showMessage("<h1>"+b.name+"</h1><p>"+b.message+"</p><p>Please contact maintainer of this site. Note: it is possible to read files only from this host.</p>");break;default:this.showMessage("<h1>"+b.name+"</h1><p>"+b.message+"</p><pre>"+b.stacktrace+'</pre><p>Please contact maintainer of this site. You can also report it <a href="#">here</a>.</p>')}};
