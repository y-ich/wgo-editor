/*! MIT license, more info: wgo.waltheri.net */(function(c,n){var h=function(a){var p=new u(JSON.parse(JSON.stringify(a.getProperties()))),b;for(b in a.children)p.appendChild(h(a.children[b]));return p},f=function(a,b){var p;if(b[a]!==n)return b[a];for(var d in b.children)if(p=f(a,b.children[d]))return p;return!1},q=function(a,b){a.push(JSON.parse(JSON.stringify(b.getProperties())));if(1<b.children.length){for(var d=[],p=0;p<b.children.length;p++){var e=[];q(e,b.children[p]);d.push(e)}a.push(d)}else b.children.length&&q(a,b.children[0])},k=function(a,
b){for(var d=b,p,e=1;e<a.length;e++)if(a[e].constructor==Array)for(var g=0;g<a[e].length;g++)p=new u(a[e][g][0]),d.appendChild(p),k(a[e][g],p);else p=new u(a[e]),d.insertAfter(p),d=p},a=function(a){return"string"==typeof a?a.replace(/\\/g,"\\\\").replace(/]/g,"\\]"):a},b=function(a,b){return String.fromCharCode(97+a)+String.fromCharCode(97+b)},r=function(a,b,d){if(b.length){d.sgf+=a;for(var p in b)d.sgf+="["+b[p]+"]"}},g=function(d,e){if(d.move){var p="";d.move.pass||(p=b(d.move.x,d.move.y));e.sgf=
d.move.c==c.B?e.sgf+("B["+p+"]"):e.sgf+("W["+p+"]")}if(d.setup){var p=[],A=[],k=[],m;for(m in d.setup)d.setup[m].c==c.B?p.push(b(d.setup[m].x,d.setup[m].y)):d.setup[m].c==c.W?A.push(b(d.setup[m].x,d.setup[m].y)):k.push(b(d.setup[m].x,d.setup[m].y));r("AB",p,e);r("AW",A,e);r("AE",k,e)}if(d.markup){p={};for(m in d.markup)p[d.markup[m].type]=p[d.markup[m].type]||[],"LB"==d.markup[m].type?p.LB.push(b(d.markup[m].x,d.markup[m].y)+":"+a(d.markup[m].text)):p[d.markup[m].type].push(b(d.markup[m].x,d.markup[m].y));
for(var l in p)r(l,p[l],e)}m=d.getProperties();for(l in m)"object"!=typeof m[l]&&(e.sgf="turn"==l?e.sgf+("PL["+(m[l]==c.B?"B":"W")+"]"):"comment"==l?e.sgf+("C["+a(m[l])+"]"):e.sgf+(l+"["+a(m[l])+"]"));if(1==d.children.length)e.sgf+="\n;",g(d.children[0],e);else if(1<d.children.length)for(l in d.children)m=d.children[l],p=e,p.sgf+="(\n;",g(m,p),p.sgf+="\n)"},l=function(){this.size=19;this.info={};this.root=new u;this.propertyCount=this.nodeCount=0};l.prototype={constructor:l,clone:function(){var d=
new l;d.size=this.size;d.info=JSON.parse(JSON.stringify(this.info));d.root=h(this.root);d.nodeCount=this.nodeCount;d.propertyCount=this.propertyCount;return d},hasComments:function(){return!!f("comment",this.root)}};l.fromSgf=function(d){return c.SGF.parse(d)};l.fromJGO=function(d){d="string"==typeof d?JSON.parse(d):d;var a=new l;a.info=JSON.parse(JSON.stringify(d.info));a.size=d.size;a.nodeCount=d.nodeCount;a.propertyCount=d.propertyCount;a.root=new u(d.game[0]);k(d.game,a.root);return a};l.prototype.toSgf=
function(){var d={sgf:"(\n;"},b={},e;for(e in this.info)"black"==e?(this.info.black.name&&(b.PB=a(this.info.black.name)),this.info.black.rank&&(b.BR=a(this.info.black.rank)),this.info.black.team&&(b.BT=a(this.info.black.team))):"white"==e?(this.info.white.name&&(b.PW=a(this.info.white.name)),this.info.white.rank&&(b.WR=a(this.info.white.rank)),this.info.white.team&&(b.WT=a(this.info.white.team))):b[e]=a(this.info[e]);this.size&&(b.SZ=this.size);b.AP||(b.AP="WGo.js:2");b.FF||(b.FF="4");b.GM||(b.GM=
"1");b.CA||(b.CA="UTF-8");for(e in b)b[e]&&(d.sgf+=e+"["+b[e]+"]");g(this.root,d);d.sgf+=")";return d.sgf};l.prototype.toJGO=function(d){var a={};a.size=this.size;a.info=JSON.parse(JSON.stringify(this.info));a.nodeCount=this.nodeCount;a.propertyCount=this.propertyCount;a.game=[];q(a.game,this.root);return d?JSON.stringify(a):a};var e=function(d){var a;d.name?(a=c.filterHTML(d.name),d.rank&&(a+=" ("+c.filterHTML(d.rank)+")"),d.team&&(a+=", "+c.filterHTML(d.team))):(d.team&&(a=c.filterHTML(d.team)),
...
"BT");f.TM=function(a,b,c,e){a.info[e]=c[0];b.BL=c[0];b.WL=c[0]};var q=/\(|\)|(;(\s*[A-Z]+(\s*((\[\])|(\[(.|\s)*?([^\\]\]))))+)*)/g,k=/[A-Z]+(\s*((\[\])|(\[(.|\s)*?([^\\]\]))))+/g,a=/[A-Z]+/,b=/(\[\])|(\[(.|\s)*?([^\\]\]))/g;c.SGF.parse=function(r){var g=[],f,e,d,m=new c.Kifu,h=null;r=r.match(q);for(var n in r)if("("==r[n])g.push(h);else if(")"==r[n])h=g.pop();else{h&&m.nodeCount++;h=h?h.appendChild(new c.KNode):m.root;f=r[n].match(k)||[];m.propertyCount+=f.length;for(var x in f){d=a.exec(f[x])[0];
e=f[x].match(b);for(var w in e)e[w]=e[w].substring(1,e[w].length-1).replace(/\\(?!\\)/g,"");if(c.SGF.properties[d])c.SGF.properties[d](m,h,e,d);else 1>=e.length&&(e=e[0]),h.parent?h[d]=e:m.info[d]=e}}return m}})(WGo);(function(c){var n=function(d,a){this.name="FileError";this.message=1==a?"File '"+d+"' is empty.":2==a?"Network error. It is not possible to read '"+d+"'.":"File '"+d+"' hasn't been found on server."};n.prototype=Error();n.prototype.constructor=n;c.FileError=n;var h=c.loadFromUrl=function(d,a){var b=new XMLHttpRequest;b.onreadystatechange=function(){if(4==b.readyState)if(200==b.status){if(0==b.responseText.length)throw new n(d,1);a(b.responseText)}else throw new n(d);};try{b.open("GET",d,!0),b.send()}catch(v){throw new n(d,
...
[c.t(b),c.filterHTML(this.kifu.info[b])]);return a},setFrozen:function(a){this.frozen=a;this.dispatchEvent({type:this.frozen?"frozen":"unfrozen",target:this})},appendTo:function(a){a.appendChild(this.element)},focus:function(){this.config.enableKeys&&this.setKeys(!0)},setKeys:function(a){document.onkeydown=a?b.bind(this):null},setWheel:function(d){!this._wheel_listener&&d?(this._wheel_listener=a.bind(this),d=void 0!==document.onmousewheel?"mousewheel":"DOMMouseScroll",this.element.addEventListener(d,
this._wheel_listener)):this._wheel_listener&&!d&&(d=void 0!==document.onmousewheel?"mousewheel":"DOMMouseScroll",this.element.removeEventListener(d,this._wheel_listener),delete this._wheel_listener)},setCoordinates:function(a){!this.coordinates&&a?(this.board.setSection(-.5,-.5,-.5,-.5),this.board.addCustomObject(c.Board.coordinates)):this.coordinates&&!a&&(this.board.setSection(0,0,0,0),this.board.removeCustomObject(c.Board.coordinates));this.coordinates=a}};g["default"]={sgf:void 0,json:void 0,
...
black:"Black",white:"White",DT:"Date",KM:"Komi",HA:"Handicap",AN:"Annotations",CP:"Copyright",GC:"Game comments",GN:"Game name",ON:"Fuseki",OT:"Overtime",TM:"Basic time",RE:"Result",RO:"Round",RU:"Rules",US:"Recorder",PC:"Place",EV:"Event",SO:"Source",none:"none",bpass:"Black passed.",wpass:"White passed."},e;for(e in l)c.i18n.en[e]=l[e]})(WGo);(function(c){var n=0,h=function(a,b,c){var g={};g.element=document.createElement("div");g.element.className="wgo-player-"+a;g.wrapper=document.createElement("div");g.wrapper.className="wgo-player-"+a+"-wrapper";g.element.appendChild(g.wrapper);b.appendChild(g.element);c||(g.element.style.display="none");return g},f=function(a){var b;if(b=this.currentLayout.layout?this.currentLayout.layout[a]:this.currentLayout[a]){this.regions[a].element.style.display="block";b.constructor!=Array&&(b=[b]);for(var c in b)this.components[b[c]]||
(this.components[b[c]]=new k.component[b[c]](this)),this.components[b[c]].appendTo(this.regions[a].wrapper),this.components[b[c]]._detachFromPlayer=!1}else this.regions[a].element.style.display="none"},q=function(a){console.log("input");if(a.target.dataset.sgfTag){switch(a.target.dataset.sgfTag){case "C":this.kifuReader.node.comment=a.target.innerText;break;case "PB":/\r?\n/.test(a.target.innerText)&&(a.target.innerText=a.target.innerText.replace(/\r?\n/g,""));this.kifu.info.black.name=a.target.innerText;
break;case "BR":/\r?\n/.test(a.target.innerText)&&(a.target.innerText=a.target.innerText.replace(/\r?\n/g,""));this.kifu.info.black.rank=a.target.innerText;break;case "PW":/\r?\n/.test(a.target.innerText)&&(a.target.innerText=a.target.innerText.replace(/\r?\n/g,""));this.kifu.info.white.name=a.target.innerText;break;case "WR":/\r?\n/.test(a.target.innerText)&&(a.target.innerText=a.target.innerText.replace(/\r?\n/g,""));this.kifu.info.white.rank=a.target.innerText;break;default:this.kifu.info[a.target.dataset.sgfTag]=
a.target.innerText}this.kifu._edited=!0}},k=c.extendClass(c.Player,function(a,b){this.config=b;for(var r in k["default"])void 0===this.config[r]&&void 0!==k["default"][r]&&(this.config[r]=k["default"][r]);for(r in c.Player["default"])void 0===this.config[r]&&void 0!==c.Player["default"][r]&&(this.config[r]=c.Player["default"][r]);this.element=a;this.element.innerHTML="";this.classes=(this.element.className?this.element.className+" ":"")+"wgo-player-main";this.element.className=this.classes;this.element.addEventListener("input",
function(a){this.dispatchEvent(a)}.bind(this),!1);this.element.id||(this.element.id="wgo_"+n++);this.dom={};this.dom.center=document.createElement("div");this.dom.center.className="wgo-player-center";this.dom.board=document.createElement("div");this.dom.board.className="wgo-player-board";this.regions={};this.regions.left=h("left",this.element);this.element.appendChild(this.dom.center);this.regions.right=h("right",this.element);this.regions.top=h("top",this.dom.center);this.dom.center.appendChild(this.dom.board);
this.regions.bottom=h("bottom",this.dom.center);this.board=new c.Board(this.dom.board,this.config.board);this.init();this.components={};window.addEventListener("resize",function(){this.noresize||this.updateDimensions()}.bind(this));this.updateDimensions();this.loadKifu(new c.Kifu,this.config.move);this.initGame();this._editable=new c.Player.Editable(this,this.board);this._editable.set(!0);this.addEventListener("input",q.bind(this))});k.prototype.appendTo=function(a){a.appendChild(this.element);this.updateDimensions()};
k.prototype.updateDimensions=function(){for(var a=window.getComputedStyle(this.element),b=[];this.element.firstChild;)b.push(this.element.firstChild),this.element.removeChild(this.element.firstChild);for(var c=parseInt(a.width),g=parseInt(a.height),k=parseInt(a.maxHeight)||0,a=0;a<b.length;a++)this.element.appendChild(b[a]);if(c!=this.width||g!=this.height||k!=this.maxHeight){this.width=c;this.height=g;this.maxHeight=k;a:if(b=this.config.layout,b.constructor==Array){c=this.height||this.maxHeight;
for(g=0;g<b.length;g++)if(!b[g].conditions||!(b[g].conditions.minWidth&&!(b[g].conditions.minWidth<=this.width)||b[g].conditions.minHeight&&c&&!(b[g].conditions.minHeight<=c)||b[g].conditions.maxWidth&&!(b[g].conditions.maxWidth>=this.width)||b[g].conditions.maxHeight&&c&&!(b[g].conditions.maxHeight>=c)||b[g].conditions.custom&&!b[g].conditions.custom.call(this))){b=b[g];break a}b=void 0}if((this.currentLayout=b)&&this.lastLayout!=this.currentLayout){this.element.className=this.currentLayout.className?
this.classes+" "+this.currentLayout.className:this.classes;for(var e in this.components)this.components[e]._detachFromPlayer=!0;f.call(this,"left");f.call(this,"right");f.call(this,"top");f.call(this,"bottom");for(e in this.components)this.components[e]._detachFromPlayer&&this.components[e].element.parentNode&&this.components[e].element.parentNode.removeChild(this.components[e].element);this.lastLayout=this.currentLayout}b=this.dom.board.clientWidth;(e=this.height||this.maxHeight)&&(e-=this.regions.top.element.offsetHeight+
this.regions.bottom.element.offsetHeight);e&&e<b?e!=this.board.height&&this.board.setHeight(e):b!=this.board.width&&this.board.setWidth(b);b=e-b;0<b?(this.dom.board.style.height=e+"px",this.dom.board.style.paddingTop=b/2+"px"):(this.dom.board.style.height="auto",this.dom.board.style.paddingTop="0");this.regions.left.element.style.height=this.dom.center.offsetHeight+"px";this.regions.right.element.style.height=this.dom.center.offsetHeight+"px";for(a in this.components)this.components[a].updateDimensions&&
this.components[a].updateDimensions()}};k.prototype.showMessage=function(a,b,r){this.info_overlay=document.createElement("div");this.info_overlay.style.width=this.element.offsetWidth+"px";this.info_overlay.style.height=this.element.offsetHeight+"px";this.info_overlay.className="wgo-info-overlay";this.element.appendChild(this.info_overlay);var g=document.createElement("div");g.className="wgo-info-message";g.innerHTML=a;a=document.createElement("div");a.className="wgo-info-close";r||(a.innerHTML=c.t("BP:closemsg"));
g.appendChild(a);this.info_overlay.appendChild(g);b?this.info_overlay.addEventListener("click",function(a){b(a)}):r||this.info_overlay.addEventListener("click",function(a){this.hideMessage()}.bind(this));this.setFrozen(!0)};k.prototype.hideMessage=function(){this.element.removeChild(this.info_overlay);this.setFrozen(!1)};k.prototype.error=function(a){if(!c.ERROR_REPORT)throw a;switch(a.name){case "InvalidMoveError":this.showMessage("<h1>"+a.name+"</h1><p>"+a.message+'</p><p>If this message isn\'t correct, please report it by clicking <a href="#">here</a>, otherwise contact maintainer of this site.</p>');
...
a.toLowerCase()&&(this.lockScroll=!1)},"data-wgo-enablekeys":function(a){"false"==a.toLowerCase()&&(this.enableKeys=!1)},"data-wgo-rememberpath":function(a){"false"==a.toLowerCase()&&(this.rememberPath=!1)},"data-wgo-allowillegal":function(a){"false"!=a.toLowerCase()&&(this.allowIllegalMoves=!0)},"data-wgo-move":function(a){var b=parseInt(a);isNaN(b)?this.move=eval("({"+a+"})"):this.move=b},"data-wgo-marklastmove":function(a){"false"==a.toLowerCase()&&(this.markLastMove=!1)},"data-wgo-diagram":function(a){a&&
("("==a[0]?this.sgf=a:this.sgfFile=a,this.enableWheel=this.enableKeys=this.markLastMove=!1,this.layout={top:[],right:[],left:[],bottom:[]})}};c.BasicPlayer=k;window.addEventListener("load",function(){for(var a=document.querySelectorAll("[data-wgo],[data-wgo-diagram]"),b=0;b<a.length;b++){var c=a[b],g,f;f={};for(var e=0;e<c.attributes.length;e++)g=c.attributes[e],k.attributes[g.name]&&k.attributes[g.name].call(f,g.value,g.name);g=new k(c,f);c._wgo_player=g}})})(WGo);(function(c,n){var h=function(){this.element=document.createElement("div")};h.prototype={constructor:h,appendTo:function(c){c.appendChild(this.element)},getWidth:function(){var c=window.getComputedStyle(this.element);return parseInt(c.width)},getHeight:function(){var c=window.getComputedStyle(this.element);return parseInt(c.height)},updateDimensions:function(){}};c.BasicPlayer.component.Component=h})(WGo);(function(){var c=function(a){this[a]={};var b=this[a];b.box=document.createElement("div");b.box.className="wgo-box-wrapper wgo-player-wrapper wgo-"+a;b.name=document.createElement("div");b.name.contentEditable="true";b.name.className="wgo-box-title";b.name.dataset.sgfTag="black"==a?"PB":"PW";b.name.innerHTML=a;b.box.appendChild(b.name);var c;c=document.createElement("div");c.className="wgo-player-info";b.box.appendChild(c);b.info={};b.info.rank=n("rank","black"==a?"BR":"WR");b.info.rank.val.innerHTML=
"-";b.info.caps=n("caps");b.info.caps.val.innerHTML="0";b.info.time=n("time");b.info.time.val.innerHTML="--:--";c.appendChild(b.info.rank.wrapper);c.appendChild(b.info.caps.wrapper);c.appendChild(b.info.time.wrapper)},n=function(a,c){var b={};b.wrapper=document.createElement("div");b.wrapper.className="wgo-player-info-box-wrapper";b.box=document.createElement("div");b.box.className="wgo-player-info-box";b.wrapper.appendChild(b.box);b.title=document.createElement("div");b.title.className="wgo-player-info-title";
b.title.innerHTML=WGo.t(a);b.box.appendChild(b.title);b.val=document.createElement("div");b.val.className="wgo-player-info-value";c&&(b.val.contentEditable="true",b.val.dataset.sgfTag=c);b.box.appendChild(b.val);return b},h=function(a){a=a.kifu.info||{};a.black?(this.black.name.innerHTML=WGo.filterHTML(a.black.name)||WGo.t("black"),this.black.info.rank.val.innerHTML=WGo.filterHTML(a.black.rank)||"-"):(this.black.name.innerHTML=WGo.t("black"),this.black.info.rank.val.innerHTML="-");a.white?(this.white.name.innerHTML=
...
a.addEventListener("kifuLoaded",h.bind(this));a.addEventListener("update",q.bind(this))});k.prototype.setPlayerTime=function(a,c){var b=Math.round(c)%60;this[a].info.time.val.innerHTML=Math.floor(c/60)+":"+(10>b?"0"+b:b)};k.prototype.updateDimensions=function(){f(this.black.name);f(this.white.name)};var a=WGo.BasicPlayer.layouts;a.right_top.right.push("InfoBox");a.right.right.push("InfoBox");a.one_column.top.push("InfoBox");a.no_comment.top.push("InfoBox");WGo.i18n.en.rank="Rank";WGo.i18n.en.caps=
"Caps";WGo.i18n.en.time="Time";WGo.BasicPlayer.component.InfoBox=k})(WGo);(function(c,n){var h=function(a){var b,c;b=a.charCodeAt(0)-97;0>b&&(b+=32);7<b&&b--;c=a.charCodeAt(1)-48;2<a.length&&(c=10*c+(a.charCodeAt(2)-48));c=this.kifuReader.game.size-c;this._tmp_mark={type:"MA",x:b,y:c};this.board.addObject(this._tmp_mark)},f=function(){this.board.removeObject(this._tmp_mark);delete this._tmp_mark},q=function(a,b){for(var c in a)a[c].className&&"wgo-move-link"==a[c].className?(a[c].addEventListener("mouseover",h.bind(b,a[c].innerHTML)),a[c].addEventListener("mouseout",f.bind(b))):
a[c].childNodes&&a[c].childNodes.length&&q(a[c].childNodes,b)},k=c.extendClass(c.BasicPlayer.component.Component,function(a){this["super"](a);this.player=a;this.element.className="wgo-commentbox";this.box=document.createElement("div");this.box.className="wgo-box-wrapper wgo-comments-wrapper";this.element.appendChild(this.box);this.comments_title=document.createElement("div");this.comments_title.className="wgo-box-title";this.comments_title.innerHTML=c.t("comments");this.box.appendChild(this.comments_title);
...
"wgo-comment-text";this.comment_text.dataset.sgfTag="C";this.comments.appendChild(this.comment_text);a.addEventListener("kifuLoaded",function(b){this.comments_title.innerHTML=c.t("comments");this.element.className="wgo-commentbox";this._update=function(a){this.setComments(a)}.bind(this);a.addEventListener("update",this._update)}.bind(this));a.notification=function(a){a?(this.notification.style.display="block",this.notification.innerHTML=a,this.is_notification=!0):(this.notification.style.display=
"none",this.is_notification=!1)}.bind(this);a.help=function(a){a?(this.help.style.display="block",this.help.innerHTML=a,this.is_help=!0):(this.help.style.display="none",this.is_help=!1)}.bind(this)});k.prototype.setComments=function(a){this.player._tmp_mark&&f.call(this.player);var b="";if(a.node.parent)this.comment_text.contentEditable="true";else{var b=a.target.getGameInfo(),k;k='<div class="wgo-info-list">'+('<div class="wgo-info-title">'+c.t("gameinfo")+"</div>");for(var g in b){var h="";/^[A-Z]{1,2}$/.test(g)&&
(h=' data-sgf-tag="'+g+'" contentEditable="true"');k+='<div class="wgo-info-item"><span class="wgo-info-label">'+b[g][0]+'</span><span class="wgo-info-value"'+h+">"+b[g][1]+"</span></div>"}b=k+"</div>";this.comment_text.contentEditable="false"}this.comment_text.innerHTML=b+this.getCommentText(a.node.comment,this.player.config.formatNicks,this.player.config.formatMoves);this.player.config.formatMoves&&this.comment_text.childNodes&&this.comment_text.childNodes.length&&q(this.comment_text.childNodes,
this.player)};k.prototype.getCommentText=function(a,b,k){return a?(a="<p>"+c.filterHTML(a).replace(/\n/g,"</p><p>")+"</p>",b&&(a=a.replace(/(<p>)([^:]{3,}:)\s/g,'<p><span class="wgo-comments-nick">$2</span> ')),k&&(a=a.replace(/\b[a-zA-Z]1?\d\b/g,'<a href="javascript:void(0)" class="wgo-move-link">$&</a>')),a):""};c.BasicPlayer["default"].formatNicks=!0;c.BasicPlayer["default"].formatMoves=!0;c.BasicPlayer.attributes["data-wgo-formatnicks"]=function(a){"false"==a.toLowerCase()&&(this.formatNicks=
!1)};c.BasicPlayer.attributes["data-wgo-formatmoves"]=function(a){"false"==a.toLowerCase()&&(this.formatMoves=!1)};c.BasicPlayer.layouts.right_top.right.push("CommentBox");c.BasicPlayer.layouts.right.right.push("CommentBox");c.BasicPlayer.layouts.one_column.bottom.push("CommentBox");c.i18n.en.comments="Comments";c.i18n.en.gameinfo="Game info";c.BasicPlayer.component.CommentBox=k})(WGo);(function(c,n){var h=c.extendClass(c.BasicPlayer.component.Component,function(a){this["super"](a);this.widgets=[];this.element.className="wgo-player-control";this.iconBar=document.createElement("div");this.iconBar.className="wgo-control-wrapper";this.element.appendChild(this.iconBar);var d,b;for(b in h.widgets)d=new h.widgets[b].constructor(a,h.widgets[b].args),d.appendTo(this.iconBar),this.widgets.push(d)});h.prototype.updateDimensions=function(){this.element.className=340>this.element.clientWidth?
"wgo-player-control wgo-340":440>this.element.clientWidth?"wgo-player-control wgo-440":"wgo-player-control"};var f=c.BasicPlayer.control={},q=function(a){a.node.parent||this.disabled?a.node.parent&&this.disabled&&this.enable():this.disable()},k=function(a){a.node.children.length||this.disabled?a.node.children.length&&this.disabled&&this.enable():this.disable()},a=function(a){(this._disabled=this.disabled)||this.disable()},b=function(a){this._disabled||this.enable();delete this._disabled};f.Widget=
function(a,d){this.element=this.element||document.createElement(d.type||"div");this.element.className="wgo-widget-"+d.name;this.init(a,d)};f.Widget.prototype={constructor:f.Widget,init:function(a,d){d&&(d.disabled&&this.disable(),d.init&&d.init.call(this,a))},appendTo:function(a){a.appendChild(this.element)},disable:function(){this.disabled=!0;-1==this.element.className.search("wgo-disabled")&&(this.element.className+=" wgo-disabled")},enable:function(){this.disabled=!1;this.element.className=this.element.className.replace(" wgo-disabled",
"");this.element.disabled=""}};f.Group=c.extendClass(f.Widget,function(a,d){this.element=document.createElement("div");this.element.className="wgo-ctrlgroup wgo-ctrlgroup-"+d.name;var b,c;for(c in d.widgets)b=new d.widgets[c].constructor(a,d.widgets[c].args),b.appendTo(this.element)});f.Clickable=c.extendClass(f.Widget,function(a,b){this["super"](a,b)});f.Clickable.prototype.init=function(a,b){var d,c=this;d=b.togglable?function(){c.disabled||(b.click.call(c,a)?c.select():c.unselect())}:function(){c.disabled||
...
widgets:[{constructor:f.Button,args:{name:"about",click:function(a){a.showMessage(c.t("about-text"))}}}]}},{constructor:f.Group,args:{name:"control",widgets:[{constructor:f.Button,args:{name:"first",disabled:!0,init:function(c){c.addEventListener("update",q.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){a.first()}}},{constructor:f.Button,args:{name:"multiprev",disabled:!0,multiple:!0,init:function(c){c.addEventListener("update",
q.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){var b=c.clone(a.kifuReader.path);b.m-=10;a.goTo(b)}}},{constructor:f.Button,args:{name:"previous",disabled:!0,multiple:!0,init:function(c){c.addEventListener("update",q.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){a.previous()}}},{constructor:f.MoveNumber},{constructor:f.Button,args:{name:"next",disabled:!0,multiple:!0,
init:function(c){c.addEventListener("update",k.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){a.next()}}},{constructor:f.Button,args:{name:"multinext",disabled:!0,multiple:!0,init:function(c){c.addEventListener("update",k.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){var b=c.clone(a.kifuReader.path);b.m+=10;a.goTo(b)}}},{constructor:f.Button,args:{name:"last",
disabled:!0,init:function(c){c.addEventListener("update",k.bind(this));c.addEventListener("frozen",a.bind(this));c.addEventListener("unfrozen",b.bind(this))},click:function(a){a.last()}}}]}}];var g=c.BasicPlayer.layouts;g.right_top.top.push("Control");g.right.right.push("Control");g.one_column.top.push("Control");g.no_comment.bottom.push("Control");g.minimal.bottom.push("Control");var g={about:"About",first:"First",multiprev:"10 moves back",previous:"Previous",next:"Next",multinext:"10 moves forward",
...
n.bind(this),this._ev_out=this._ev_out||h.bind(this),this.board.addEventListener("click",this._ev_click),this.board.addEventListener("mousemove",this._ev_move),this.board.addEventListener("mouseout",this._ev_out),this.editMode=!0;else if(this.editMode&&!f){this.originalReader.goTo(this.player.kifuReader.path);f=this.originalReader;for(var q=this.player.kifuReader.getPosition(),k=this.originalReader.getPosition(),a=q.size,b=[],r=[],g=0;g<a*a;g++)q.schema[g]&&!k.schema[g]?r.push({x:Math.floor(g/a),
y:g%a}):q.schema[g]!=k.schema[g]&&b.push({x:Math.floor(g/a),y:g%a,c:k.schema[g]});f.change={add:b,remove:r};this.player.kifuReader=this.originalReader;this.player.update(!0);this.board.removeEventListener("click",this._ev_click);this.board.removeEventListener("mousemove",this._ev_move);this.board.removeEventListener("mouseout",this._ev_out);this.editMode=!1}};c.Player.Editable.prototype.play=function(f,h){if(!this.player.frozen)if(this.player.kifuReader.game.isValid(f,h))this.player.kifuReader.node.appendChild(new c.KNode({move:{x:f,
y:h,c:this.player.kifuReader.game.turn},_edited:!0})),this.player.kifu._edited=!0,this.player.next(this.player.kifuReader.node.children.length-1);else{var k=this.player.kifuReader.node;0===k.children.length&&k.move.x===f&&k.move.y===h&&(this.player.previous(),k.remove())}};c.i18n.en.editmode="Edit mode"})(WGo);(function(c){var n=function(c,a,b,f){this.originalPosition=c;this.position=c.clone();this.board=a;this.komi=b;this.output=f},h=n.state={UNKNOWN:0,BLACK_STONE:1,WHITE_STONE:-1,BLACK_CANDIDATE:2,WHITE_CANDIDATE:-2,BLACK_NEUTRAL:3,WHITE_NEUTRAL:-3,NEUTRAL:4},f=function(c,a,b,h,g){var k=c.get(a,b);void 0!==k&&k!=h&&k!=g&&(c.set(a,b,h),f(c,a-1,b,h,g),f(c,a,b-1,h,g),f(c,a+1,b,h,g),f(c,a,b+1,h,g))},q=function(c,a,b,h,g){var k=a.get(b,h);c.get(b,h)!=k&&(c.set(b,h,k),q(c,a,b-1,h,g),q(c,a,b,h-1,g),q(c,a,b+
1,h,g),q(c,a,b,h+1,g))};n.prototype.start=function(){this.calculate();this.saved_state=this.board.getState();this.displayScore();this._click=function(k,a){var b=this.originalPosition.get(k,a);b==c.W?this.position.get(k,a)==h.WHITE_STONE?f(this.position,k,a,h.BLACK_CANDIDATE,h.BLACK_STONE):(q(this.position,this.originalPosition,k,a,h.BLACK_STONE),this.calculate()):b==c.B?this.position.get(k,a)==h.BLACK_STONE?f(this.position,k,a,h.WHITE_CANDIDATE,h.WHITE_STONE):(q(this.position,this.originalPosition,
k,a,h.WHITE_STONE),this.calculate()):(b=this.position.get(k,a),b==h.BLACK_CANDIDATE?this.position.set(k,a,h.BLACK_NEUTRAL):b==h.WHITE_CANDIDATE?this.position.set(k,a,h.WHITE_NEUTRAL):b==h.BLACK_NEUTRAL?this.position.set(k,a,h.BLACK_CANDIDATE):b==h.WHITE_NEUTRAL&&this.position.set(k,a,h.WHITE_CANDIDATE));this.board.restoreState({objects:c.clone(this.saved_state.objects)});this.displayScore()}.bind(this);this.board.addEventListener("click",this._click)};n.prototype.end=function(){this.board.restoreState({objects:c.clone(this.saved_state.objects)});
this.board.removeEventListener("click",this._click)};n.prototype.displayScore=function(){for(var k=[],a=[],b=[],f=[],g=[],l=0;l<this.position.size;l++)for(var e=0;e<this.position.size;e++)s=this.position.get(l,e),t=this.originalPosition.get(l,e),s==h.BLACK_CANDIDATE?k.push({x:l,y:e,type:"mini",c:c.B}):s==h.WHITE_CANDIDATE?a.push({x:l,y:e,type:"mini",c:c.W}):s==h.NEUTRAL&&b.push({x:l,y:e}),t==c.W&&s!=h.WHITE_STONE?g.push({x:l,y:e,type:"outline",c:c.W}):t==c.B&&s!=h.BLACK_STONE&&f.push({x:l,y:e,type:"outline",
c:c.B});for(l=0;l<f.length;l++)this.board.removeObjectsAt(f[l].x,f[l].y);for(l=0;l<g.length;l++)this.board.removeObjectsAt(g[l].x,g[l].y);this.board.addObject(g);this.board.addObject(f);this.board.addObject(k);this.board.addObject(a);b="<p style='font-weight: bold;'>"+c.t("RE")+"</p>";l=k.length+g.length+this.originalPosition.capCount.black;e=a.length+f.length+this.originalPosition.capCount.white+parseFloat(this.komi);b+="<p>"+c.t("black")+": "+k.length+" + "+(g.length+this.originalPosition.capCount.black)+
" = "+l+"</br>";b+=c.t("white")+": "+a.length+" + "+(f.length+this.originalPosition.capCount.white)+" + "+this.komi+" = "+e+"</p>";b=l>e?b+("<p style='font-weight: bold;'>"+c.t("bwin",l-e)+"</p>"):b+("<p style='font-weight: bold;'>"+c.t("wwin",e-l)+"</p>");this.output(b)};n.prototype.calculate=function(){var c,a,b,f,g,l;c=this.position;for(l=!0;l;){l=!1;for(var e=0;e<c.size;e++)for(var d=0;d<c.size;d++)if(a=c.get(d,e),a==h.UNKNOWN||a==h.BLACK_CANDIDATE||a==h.WHITE_CANDIDATE){b=[c.get(d-1,e),c.get(d,
e-1),c.get(d+1,e),c.get(d,e+1)];g=f=!1;for(var m=0;4>m;m++)b[m]==h.BLACK_STONE||b[m]==h.BLACK_CANDIDATE?f=!0:b[m]==h.WHITE_STONE||b[m]==h.WHITE_CANDIDATE?g=!0:b[m]==h.NEUTRAL&&(g=f=!0);b=!1;f&&g?b=h.NEUTRAL:f?b=h.BLACK_CANDIDATE:g&&(b=h.WHITE_CANDIDATE);b&&a!=b&&(l=!0,c.set(d,e,b))}}};c.ScoreMode=n;c.BasicPlayer&&c.BasicPlayer.component.Control&&c.BasicPlayer.component.Control.menu.push({constructor:c.BasicPlayer.control.MenuItem,args:{name:"scoremode",togglable:!0,click:function(f){if(this.selected)return f.setFrozen(!1),
this._score_mode.end(),delete this._score_mode,f.notification(),f.help(),!1;f.setFrozen(!0);f.help("<p>"+c.t("help_score")+"</p>");this._score_mode=new c.ScoreMode(f.kifuReader.game.position,f.board,f.kifu.info.KM||.5,f.notification);this._score_mode.start();return!0}}});c.i18n.en.scoremode="Count score";c.i18n.en.score="Score";c.i18n.en.bwin="Black wins by $ points.";c.i18n.en.wwin="White wins by $ points.";c.i18n.en.help_score="Click on stones to mark them dead or alive. You can also set and unset territory points by clicking on them. Territories must be completely bordered."})(WGo);(function(c,n){var h={active:!0,query:{}},f=function(c){try{h.query=JSON.parse('{"'+window.location.hash.substr(1).replace("=",'":')+"}")}catch(a){h.query={}}};window.addEventListener("hashchange",function(){if(""!=window.location.hash&&h.active){f();for(var c in h.query){var a=document.getElementById(c);a&&a._wgo_player&&a._wgo_player.goTo(q)}}});window.addEventListener("DOMContentLoaded",function(){""!=window.location.hash&&h.active&&f()});window.addEventListener("load",function(){if(""!=window.location.hash&&
h.active)for(var c in h.query){var a=document.getElementById(c);if(a&&a._wgo_player){a.scrollIntoView();break}}});var q=function(){if(h.query[this.element.id])return h.query[this.element.id]["goto"]};c.Player["default"].move=q;c.BasicPlayer&&c.BasicPlayer.component.Control&&c.BasicPlayer.component.Control.menu.push({constructor:c.BasicPlayer.control.MenuItem,args:{name:"permalink",click:function(f){var a=location.href.split("#")[0]+"#"+f.element.id+'={"goto":'+JSON.stringify(f.kifuReader.path)+"}";
f.showMessage("<h1>"+c.t("permalink")+'</h1><p><input class="wgo-permalink" type="text" value=\''+a+'\' onclick="this.select(); event.stopPropagation()"/></p>')}}});c.Player.permalink=h;c.i18n.en.permalink="Permanent link"})(WGo);
